...
def visualize(true_y, pred_y, t1, t2, itr):
    ...
    if args.viz:
        ...
        i = Np-1
        ax_traj.plot(true_y.cpu().numpy()[:, 2 * i], true_y.cpu().numpy()[:, 2 * i + 1], color=colors[i], ls='-', label='SFM')
        ...
        for i in range(Np-1):
            dist_true = np.sqrt(np.square(true_y.cpu().numpy()[:len(t1), 2*i] - right) + np.square(true_y.cpu().numpy()[:len(t1), 2*i+1]))
            ax_dist.plot(t1.cpu().numpy(), dist_true, color=colors[i], ls='-')
            dist_pred = np.sqrt(np.square(pred_y.cpu().numpy()[:len(t2), i, 0] - right) + np.square(pred_y.cpu().numpy()[:len(t2), i, 1]))
            ax_dist.plot(t2.cpu().numpy(), dist_pred, color=colors[i], ls='--')
        i = Np-1
        dist_true = np.sqrt(
            np.square(true_y.cpu().numpy()[:len(t1), 2 * i] - right) + np.square(true_y.cpu().numpy()[:len(t1), 2 * i + 1]))
        ax_dist.plot(t1.cpu().numpy(), dist_true, color=colors[i], ls='-', label='SFM')
        dist_pred = np.sqrt(
        ...
        plt.savefig(saveDir + '/png_all/{:03d}.eps'.format(itr), format='eps', bbox_inches='tight', pad_inches=0.1)
        plt.draw()
        plt.pause(0.001)
...
def showprocess(true_y, pred_y, tt):
    ...
    ax_true.set_title("SFM")
    ax_true.set_xticks([])
    ax_true.set_yticks([])

    plt.suptitle("$t = $" + str(tt / 100.0) + " $s$", y=0.78, fontsize=16)
    plt.draw()
    plt.savefig(saveDir + '/png2/' + str(tt) + '.png', bbox_inches='tight', pad_inches=0.1)
    plt.pause(0.001)
...
def imgs2video(length):
    ...
    imgs_dir = saveDir + '/png2/'
    save_name = saveDir + '/video2.avi'
...
def draw_model(saveDir):
    dist2 = np.linspace(0.3, 1.3, 70)
    f2 = 2000.0 * np.exp((0.3 - dist2) / 0.08)
    f2 = np.log(f2)
    net2 = nn.Sequential(
        nn.Linear(1, 1),
    )
    dist3 = np.linspace(0.6, 1.3, 70)
    f3 = 2000.0 * np.exp((0.6 - dist3) / 0.08)
    net3 = nn.Sequential(
        nn.Linear(1, 16),
        nn.ReLU(),
        nn.Linear(16, 8),
    ...
    net2.load_state_dict(torch.load(saveDir + '/net2_parameter.pkl'))
    net3.load_state_dict(torch.load(saveDir + '/net3_parameter.pkl'))
    f1 = net2(torch.tensor(dist2.reshape(-1, 1)).float())
    fig = plt.figure()
    plt.plot(dist2, f2, color='b', label='true')
    plt.plot(dist2, f1.detach().numpy(), color='r', label='pred')
    plt.legend()
    plt.savefig(saveDir + '/net2.png')
    f1 = net3(torch.tensor(dist3.reshape(-1, 1)).float())
    fig = plt.figure()
    plt.plot(dist3, f3, color='b', label='true')
    plt.plot(dist3, f1.detach().numpy(), color='r', label='pred')
    plt.legend()
    plt.savefig(saveDir + '/net3.png')
    net1 = torch.load(saveDir + '/net1_parameter.pkl')
    return net1
...
if __name__ == "__main__":
    ...
    for k in range(1):
        # print(k)
        k = 204
        vel = np.loadtxt('sfm_data/samples80/round' + str(k+1) + '/velocity.txt')
        coord = np.loadtxt('sfm_data/samples80/round' + str(k+1) + '/coord.txt')
        vel = vel.reshape(-1, Np, 2)
        vel = vel.reshape(-1, 2 * Np)
        coord = coord.reshape(-1, Np, 2)
        test = coord[:, :, 0]
        count = np.where(test > 5.0)
        Nt = count[0][0] + 1
        coord = coord.reshape(-1, 2 * Np)
        data = np.concatenate((coord, vel), axis=1)
        data = torch.tensor(data)
        for i in range(Np):
            s = torch.cat([data[0, 2 * i:2 * i + 2].reshape(-1, 2), data[0, 2 * (i + Np):2 * (i + Np + 1)].reshape(-1, 2)],
                          1)
            if i == 0:
                restX = data[0, 2 * i + 2:2 * Np]
            elif i == Np - 1:
                restX = data[0, 0:2 * i]
            else:
                restX = torch.cat([data[0, 0:2 * i].reshape(1, -1), data[0, 2 * (i + 1):2 * Np].reshape(1, -1)], 1)

            pickindices = np.where(restX.reshape(-1, 2)[:, 0] < right)[0]
            restX = restX.reshape(-1, 2)[pickindices]
            ux = restX.reshape(1, -1) - data[0, 2 * i:2 * i + 2].reshape(1, -1).repeat(1, len(restX))
            length = np.shape(ux)[1]
            if length < 2 * Nknn:
                restU = 10.0 * torch.ones(2 * Nknn - length).reshape(1, -1)
                ux = torch.cat([ux, restU], 1)
    ...
    ##################################################################
        ...
        # #
        # aver_num1 = np.load(saveDir + '/aver_num1.npy')
        # aver_num2 = np.load(saveDir + '/aver_num2.npy')
        ...
        # ax1.plot(X1, (Np-aver_num1)/Np, ls='-', label='SFM')
        # ax1.plot(X1, (Np-aver_num2)/Np, ls='--', label='ODE-Net')
        # ax1.set_xlabel("t (s)")
        # ax1.set_ylabel("$N_{out} / N$")
        # ax1.legend()
        # # plt.savefig(saveDir + "/png_all/averOutNum_for_" + str(200) + "_rounds.png", bbox_inches='tight', pad_inches=0.1)
        #
        # ev1 = np.load(saveDir + '/ev1.npy')
        # ev2 = np.load(saveDir + '/ev2.npy')
        # bins = np.linspace(10, 18, 17)
        # ax2.hist([ev1, ev2], bins, density=True, label=['SFM', 'ODE-Net'])
        ...
        # plt.savefig(saveDir + "/png_all20/hist_for_" + str(200) + "_rounds.png", bbox_inches='tight',
        #             pad_inches=0.1)


    if os.path.exists(saveDir + '/png2'):
        shutil.rmtree(saveDir + '/png2')
    makedirs(saveDir + '/png2')
